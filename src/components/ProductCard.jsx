...
const ProductCard = ({ product, onToggleFavorite }) => {
    return (
        <div className="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 flex flex-col h-full hover-lift premium-shadow-hover transition-all duration-300 cursor-pointer">
            <div className="relative h-48 bg-[#F5F5F5] rounded-2xl mb-4 flex items-center justify-center group">
                ...
                <img
                    src={product.image}
                    alt={product.name}
                    className="h-40 object-contain mix-blend-multiply group-hover:scale-110 transition-transform duration-500"
                />
